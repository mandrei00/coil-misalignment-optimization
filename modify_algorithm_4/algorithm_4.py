...
def generate_coil(r_in, r_out, n, r_turn):
    ...
    for _ in range(n - 2):
        r_in_i = mutation_lb(
            start=r_in + 2 * r_turn, finish=r_out - 2 * r_turn
        )
        coil = np.append(coil, r_in_i)

    coil = np.append(coil, r_out)
    new_coil = np.array([r_in])
...
def hill_climbing(coil_t, coil_r, distance, min_max_m):
    ...
    # generating the transmitting coil with unevenly distributed turns
    coil_tq = generate_coil(
        r_in=r_in_tq,
        r_out=r_out_tq,
        n=n_tq,
        r_turn=coil_t[3]
    ...
    r_in_rq = mutation_lb(
        start=1e-3, finish=coil_r[1] - 2 * coil_r[3]
    )
    k_r_max = int((r_in_rq - coil_r[0]) / (2 * coil_r[3]) + 1)
    k_rq = np.random.randint(2, 3 if k_r_max <= 2 else k_r_max)

    # generating the receiving coil with unevenly distributed turns
    coil_rq = generate_coil(
        r_in=r_in_rq,
        r_out=coil_r[1],
        n=k_rq,
        r_turn=coil_r[3]
...
def stochastic_optimization_algorithm_4(**kwargs):
    ...
    # Step 7. Calculation of R outT max.
